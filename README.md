...
> **使用前必读**
>
> - **封号风险**：这是 DLL 注入型工具。向游戏进程注入 DLL，在带反作弊的联机游戏里
>   （EasyAntiCheat、BattlEye、Vanguard 等）**可能被判定作弊并封号**。
>   请只用于单机游戏，或先确认该联机模式不检测注入。
> - **杀毒拦截**：DLL 未做数字签名，Windows SmartScreen 与杀毒软件会拦截或隔离，
>   是否放行请自行判断。
> - **先备份**：安装会向游戏目录写入／改名 DLL，动手前请自行备份游戏文件。
> - **后果自负**：本汉化版仅供学习交流，与 OptiScaler 原作者无关；
